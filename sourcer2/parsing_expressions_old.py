...
#     ProgramBuilder,
# )
from outsourcer import Code, CodeBuilder, Val
...
Tup = lambda *items = Code(items)
Yield = lambda obj: Code('(yield ', Val(obj), ')')
...
class Expr:
    ...
    def compile(self, out):
        if out.has_available_blocks(self.num_blocks):
            self._compile(out)
        else:
            func, params = _functionalize(out, self, is_generator=False)
            out += Val((STATUS, RESULT, POS)) << func(*params)


def _add_comment(out, expr):
    content = str(expr)
    ...
    out += Code(
        Code('"""'),
        *[Code(x) for x in content.replace('"""', '\\"\\"\\"').split('\n')],
        Code('"""'),
    )


def _functionalize(out, expr, is_generator=False):
...
class Sep(Expr):
    ...
    def _compile(self, out):
        out += Code(f'# <{self.__class__.__name__}>')
        _add_comment(out, self)
        staging = out.var('staging', Code('[]'))
        checkpoint = out.var('checkpoint', POS)

        with out.loop():
            with _if_fails(out, self.expr):
            ...
            with _if_fails(out, self.separator):
                out += BREAK
...
class Apply(Expr):
    ...
    def _compile(self, out):
        ...
        with _if_succeeds(out, self.expr1):
            first = out.var('func' if self.apply_left else 'arg', RESULT)
            with _if_succeeds(out, self.expr2):
    ...
    def _compile(self, out):
        ...
        for arg in self.args:
            ...
            if isinstance(expr, Ref):
                value = Code(expr.resolved)
            elif isinstance(expr, PythonExpression):
                value = Code(expr.source_code)
            else:
                func, params = _functionalize(out, expr, is_generator=True)
                if len(params) > 2:
                    value = code._ParseFunction(func, tuple(params[2:]), ())
                    value = out.var('arg', value)
                else:
                    value = func

                if isinstance(expr, StringLiteral):
                    value = code._wrap_string_literal(expr.value, value)
                    value = out.var('arg', value)
        ...
        func = code._ParseFunction(Code(self.func.resolved), tuple(args), tuple(kwargs))
        func = out.var('func', func)
        out += Tup(STATUS, RESULT, POS) << Yield(Tup(CALL, func, POS))
        out += Code(f'# </{self.__class__.__name__}>')
...
class Choice(Expr):
    ...
    def _compile(self, out):
        ...
        if needs_err:
            farthest_pos = out.var('farthest_pos')
            out += backtrack << farthest_pos << POS
            farthest_err = out.var('farthest_err', Code(_error_func_name(self)))
        else:
            out += backtrack << POS

        with out.breakable():
            for i, expr in enumerate(self.exprs):
                out += Code(f'# Option {i+1}:')

                with _if_succeeds(out, expr):
                    out += BREAK
...
class Expect(Expr):
    ...
    def _compile(self, out):
        ...
        with _if_succeeds(out, self.expr):
            out += POS << backtrack
...
class ExpectNot(Expr):
    ...
    def _compile(self, pb):
        pb(Code(f'# <{self.__class__.__name__}>'))
        ...
        with pb.IF(STATUS):
            pb(STATUS << False)
            pb(RESULT << Code(_error_func_name(self)))
...
class Fail(Expr):
    ...
    def _compile(self, pb):
        pb(Code(f'# <{self.__class__.__name__}>'))
        _add_comment(pb, self)
        pb(STATUS << False, RESULT << Code(_error_func_name(self)))
        pb(Code(f'# </{self.__class__.__name__}>'))
    ...
    def _compile(self, pb):
        ...
        with pb.loop():
            ...
            if self.max_len is not None:
                with pb.IF(code.len(staging) == Code(self.max_len)):
                    pb(BREAK)
        ...
        if not self.min_len or self.min_len == '0':
            ...
            if self.min_len == 1 or self.min_len == '1':
                ...
                condition = code.len(staging) >= Code(self.min_len)
...
class Ref(Expr):
    ...
    def _compile(self, pb):
        pb(Code(f'# <{self.__class__.__name__} name={self.name!r}>'))
        func = self.name if self._resolved is None else self._resolved
        pb(Tup(STATUS, RESULT, POS) << Yield(Tup(CALL, Code(func), POS)))
        pb(Code(f'# </{self.__class__.__name__}>'))
...
class RegexLiteral(Expr):
    ...
    def _compile(self, pb):
        pb(Code(f'# <Regex pattern={self.pattern!r}>'))
...
class Rule(Expr):
    ...
    def _compile(self, pb):
        ...
        with pb.global_section():
            ...
            pb(
                Code(f'{self.name} = Rule({self.name!r}, {entry_name}, """'),
                *[Code(f'    {x}') for x in definition.split('\n')],
                Code('""")'),
            )
...
class Seq(Expr):
    ...
    def __init__(self, *exprs, names=None, constructor=None):
        ...
        if constructor is None:
            self.constructor = lambda *items: Code('[', *items, ']')
        else:
            self.constructor = Code(self.constructor)
    ...
    def _compile(self, pb):
        ...
        with pb.breakable():
            ...
            for name, expr in zip(self.names, self.exprs):
                ...
                item = pb.var('item') if name is None else Code(name)
                pb(item << RESULT)
                items.append(item)

            pb(RESULT << self.constructor(*items))
...
class StringLiteral(Expr):
    ...
    def _compile(self, pb):
        pb(Code(f'# <String value={self.value!r}>'))
        if not self.value:
            pb(STATUS << True, RESULT << Val(''))
            pb(Code('# </String>'))
            return
...
class LeftAssoc(OperatorPrecedenceRule):
    ...
    def _compile(self, pb):
        ...
        with pb.loop():
            ...
            with pb.ELSE():
                pb(staging << code.Infix(staging, operator, RESULT))
...
class PythonExpression(Expr):
    ...
    def _compile(self, pb):
        pb(RESULT << Code(self.source_code), STATUS << True)
...
def _skip_ignored(pos):
    return Yield(Tup(CALL, Code(_cont_name('_ignored')), pos))[2]
...
def generate_source_code(docstring, nodes):
    ...
    if docstring:
        safe = docstring.replace('\\', '\\\\') .replace('"""', '\\"\\"\\"'))
        out += f'"""\n{safe}\n"""\n\n'

    out += Code('from collections import namedtuple as _nt')
    out += Code('from re import compile as _compile_re, IGNORECASE as _IGNORECASE')
    ...
    pb(Code(Template(_main_template).substitute(
        CALL=CALL,
    ...
    def maybe_compile_error_message(rule, expr):
        ...
        with pb.global_function(_error_func_name(expr), [str(TEXT), str(POS)]):
            with pb.IF(code.len(TEXT) <= POS):
                pb(
                    code.title << Val('Unexpected end of input.'),
                    code.line << Val(None),
                    code.col << Val(None),
                )
            ...
            with pb.ELSE():
                pb(
                    Tup(code.line, code.col) << code._get_line_and_column(TEXT, POS),
                    code.excerpt << code._extract_excerpt(TEXT, POS, code.col),
                    code.title << Code(
            ...
            pb(
                ...
                Val(expr.complain()),
                Code(')'),
                Code('raise ', code.ParseError(
                    code.title + code.details, POS, code.line, code.col,
                )),
            )
